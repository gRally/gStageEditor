        ...
        */
        public bool CreateOrConfigureRigidBody(ref RigidBody rb, ref BulletSharp.Math.Vector3 localInertia, CollisionShape cs, MotionState motionState)
        {
            ...
            if (rb == null)
            {
                ...
                // gRally rbInfo.RollingFriction = _rollingFriction;
        ...
        void Update()
        {
            if (m_motionState != null && m_motionState.IsChanged)
            {
                transform.position = m_motionState.NewPosition;
                transform.rotation = m_motionState.NewRotation;
                // OPT transform.localScale = m_motionState.NewScale;
                m_motionState.IsChanged = false;
            }
        }
        // gRally 
        ...
        {
            // gRally BPhysicsWorld.Get().AddRigidBody(this);
            var bt = BPhysicsWorld.Get();
            if (bt != null)
            {
                bt.AddRigidBody(this);
            }
        }
