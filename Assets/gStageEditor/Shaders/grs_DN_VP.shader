Shader "gRally/grs_DN_VP"
{
	Properties
	{
		...
		[Enum(Add,0,Mul,1)] _BlendMode ("Blend Mode", Float) = 0
	}
	...
	SubShader
	{
		...
		{
			// diffuse color
			fixed4 cW = tex2D(_MainTex, IN.main1) * _Color;
			if (_BlendMode == 0)		o.Albedo = cW.rgb + IN.vColor;
			else if (_BlendMode == 1)	o.Albedo = cW.rgb * IN.vColor;
